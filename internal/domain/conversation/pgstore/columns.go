...
const messageColumnList = `id, conversation_id, sender_account_id, sender_device_id, client_message_id, sequence, kind, status, payload_key_id, payload_algorithm, payload_nonce, payload_ciphertext, payload_aad, payload_metadata, reply_conversation_id, reply_message_id, reply_sender_account_id, reply_kind, reply_snippet, forward_conversation_id, forward_message_id, forward_sender_account_id, forward_kind, forward_snippet, thread_id, silent, pinned, disable_link_previews, view_count, deliver_at, metadata, created_at, updated_at, edited_at, deleted_at`
