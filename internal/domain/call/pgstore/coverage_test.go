...
func TestScanHelpers(t *testing.T) {
	...
	callRow, err := scanCall(scanRow{values: []any{
		"call-1", "conv-1", "acc-a", "sess-1", true, call.StateActive, call.EndReasonEnded,
		now, now, now, now,
	}})
	require.NoError(t, err)
	...
	participantRow, err := scanParticipant(scanRow{values: []any{
		"call-1", "acc-b", "dev-b", call.ParticipantStateJoined, false, true, false, true, now, nil, now,
	}})
	require.NoError(t, err)
...
func TestCallInviteParticipantQueries(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT .* FROM "call"\."call_calls" WHERE call_id = \$1`).
		...
		WillReturnRows(sqlmock.NewRows([]string{
			...
			"call-1", "conv-1", "acc-a", "sess-1", true, call.StateActive, "", now, nil, nil, now,
		))
	...
	mock.ExpectQuery(`SELECT .* FROM "call"\."call_calls".*WHERE conversation_id = \$1.*ORDER BY started_at DESC, call_id ASC`).
		...
		WillReturnRows(sqlmock.NewRows([]string{
			...
			"call-1", "conv-1", "acc-a", "", true, call.StateActive, "", now, nil, nil, now,
		))
	...
	mock.ExpectQuery(`(?s)INSERT INTO "call"\."call_participants".*RETURNING`).
		WithArgs("call-1", "acc-b", "dev-b", call.ParticipantStateJoined, false, false, true, false, now, nil, now).
		WillReturnRows(sqlmock.NewRows([]string{
			...
			"call-1", "acc-b", "dev-b", call.ParticipantStateJoined, false, false, true, false, now, nil, now,
		))
	mock.ExpectCommit()
