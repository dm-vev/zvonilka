...
max_bundle_bytes, max_fragment_bytes, allowed_conversation_kinds, allowed_event_families, allowed_message_kinds, created_at, updated_at, last_healthy_at, last_error`
	bundleColumnList = `
...
func (s *Store) SaveLink(ctx context.Context, link federation.Link) (federation.Link, error) {
	...
	rawMessageKinds, err := encodeMessageKinds(link.AllowedMessageKinds)
	if err != nil {
		return federation.Link{}, err
	}
...
ON CONFLICT (id) DO UPDATE SET
	...
	allowed_message_kinds = EXCLUDED.allowed_message_kinds,
	created_at = EXCLUDED.created_at,
	updated_at = EXCLUDED.updated_at,
