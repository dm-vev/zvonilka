...
func TestBeginLoginMasksUnicodeEmailWithoutBreakingUTF8(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	store := teststore.NewMemoryStore()
	now := time.Date(2026, time.March, 23, 20, 12, 0, 0, time.UTC)
	svc, err := identity.NewService(
		store,
		identity.NoopCodeSender{},
		identity.WithNow(func() time.Time { return now }),
	...
	_, _, err = svc.CreateAccount(ctx, identity.CreateAccountParams{
		Username:    "unicode-email-user",
		DisplayName: "Unicode Email User",
	...
	if err != nil {
		t.Fatalf("begin login by unicode email: %v", err)
	}
	if len(targets) != 1 {
		t.Fatalf("expected 1 login target, got %d", len(targets))
	}
