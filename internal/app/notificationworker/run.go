...
// Run boots the notification worker service.
func Run(ctx context.Context) (err error) {
	cfg, err := config.Load("notificationworker")
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("initialize notification worker app: %w", err)
	}
	defer func() {
...
func finalizeRun(ctx context.Context, app *app, runErr error) error {
	...
	wrappedCloseErr := fmt.Errorf("close notification worker app: %w", closeErr)
	if runErr != nil {
