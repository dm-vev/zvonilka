...
func (w *worker) Run(ctx context.Context, logger *slog.Logger) error {
	...
	for {
		if err := w.processOnce(ctx); err != nil && !errors.Is(err, context.Canceled) {
			logger.ErrorContext(ctx, "process meshcore bridge batch", "err", err)
		}
...
func (w *worker) processOnce(ctx context.Context) error {
	...
	pulled, err := w.bridge.PullFragments(ctx, w.settings.PeerServerName, w.settings.LinkName, w.settings.BatchSize)
	if err != nil {
		return err
	}
	if pulled != nil && len(pulled.GetFragments()) > 0 {
		sentIDs, err := w.adapter.Send(ctx, w.settings.PeerServerName, w.settings.LinkName, pulled.GetLink(), pulled.GetFragments())
		if err != nil {
			return err
		}
		if len(sentIDs) > 0 {
