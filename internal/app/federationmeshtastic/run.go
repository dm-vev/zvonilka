...
// Run boots the Meshtastic federation bridge daemon.
func Run(ctx context.Context) (err error) {
	cfg, err := config.Load("federationmeshtastic")
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("initialize federation meshtastic app: %w", err)
	}
	defer func() {
		err = finalizeRun(app, err)
	}()
...
func finalizeRun(app *app, runErr error) error {
	if app == nil {
		return runErr
	}

	closeErr := closeApp(app)
	if closeErr == nil {
		return runErr
	}

	wrappedCloseErr := fmt.Errorf("close federation meshtastic app: %w", closeErr)
	if runErr != nil {
