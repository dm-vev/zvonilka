...
// Run boots the federation worker service.
func Run(ctx context.Context) (err error) {
	cfg, err := config.Load("federationworker")
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("initialize federation worker app: %w", err)
	}
	defer func() {
...
func finalizeRun(ctx context.Context, app *app, runErr error) error {
	...
	wrappedCloseErr := fmt.Errorf("close federation worker app: %w", closeErr)
	if runErr != nil {
